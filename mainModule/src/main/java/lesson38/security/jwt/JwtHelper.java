...
public class JwtHelper {
    ...
    public String generateToken(String issuer, String login, String[] rights) {
        ...
        JWTClaimsSet claimsSet = new JWTClaimsSet.Builder()
                .issuer(issuer) // iss - здесь кто выдал токен
                .subject(login) // sub - здесь кто выпустил токен
                .claim("login", login)
                .claim("rights", rights)
    ...
    private Pair<Date, Date> getIssueAndExpirationTimes() {
        Instant issuedAt = Instant.now().truncatedTo(ChronoUnit.MILLIS); // здесь фиксируется время, когда токен выдан и
        // откоторого отсчитывается время истечения срока
        Instant expirationAt = issuedAt.plus(Duration.ofDays(1)); // время на которое выдаётся токен - здесь на 1 день
