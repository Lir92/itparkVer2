...
public class IVehicleProxyRunner {
    ...
    public static void main(String[] args) {
        ...
        IVehicle vehicle = (IVehicle) Proxy.newProxyInstance(IVehicleProxyRunner.class.getClassLoader(), //
                new Class[]{IVehicle.class},                                                            // тут создаётся proxy объект на интерфейсе IVehicle
                new ProxyHandler(car));                                                                //

        System.out.println("Пример использования аннотации в качестве параметра метода: " + vehicle.getHeight());
        ...
        System.out.println("Пример использования аннотации в качестве параметра метода: " + proxyGolf.getHeight());
        System.out.println("Пример использования аннотации над методом: " + proxyGolf.getMarkAndModel(125));
    }

    // ниже идёт логика работы динамического proxy
    @RequiredArgsConstructor
    private static class ProxyHandler implements InvocationHandler {

        private final IVehicle origin; // при помощи него мы достаём информацию об аннотациях, определённых в классе объекта (Car)

        @Override
        public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
            Method declaredMethod = origin.getClass().getDeclaredMethod(method.getName(), method.getParameterTypes()); // здесь получаем ссылку на метод, который есть в оригинальном классе
