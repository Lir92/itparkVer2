...
//        em.getTransaction().begin();
        ...
        final List resultList = selectAllQuery.getResultList(); // получаем все искомые записи в БД
//        em.getTransaction().commit();
    ...
    public Author save(@Valid Author author) {
        if (author.getId() == 0) { // если id нет, то выполнится метод persist, т.е. добавится необходимый id
            em.persist(author);
            return author;
        } else {
            return em.merge(author); // иначе возвращается искомый автор
        }
    }
