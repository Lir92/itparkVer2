...
public class UnzipRunner {
    ...
    // Если на архиве есть пароль, то для разархивации он указывается вторым параметром в Modify options -> Program Arguments
    // через пробел.
    ...
    public static void main(String[] args) {
        ...
        Arrays.stream(args).findFirst().ifPresentOrElse(
                file -> {
                    String zip = archiveService.unzip(file, args.length >= 2 ? args[1] : null).orElseThrow(() -> new IllegalStateException("Невозможно выполнить архивацию"));
                    System.out.printf("Архиватор успешно завершил свою работу. Результат %s", zip);
                },
                () -> {
                    System.out.println("Необходимо указать разархивируемый файл в качестве параметра");
                    System.exit(0);
                }
        );
    }
