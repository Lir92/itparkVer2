...
public class JobConfig {
    ...
    public RepositoryItemReader<lesson42.model.sql.Author> authorReader(AuthorJpaRepository authorRepository) {
        ...
        Map<String, Direction> sorts = new HashMap<>(); // сортировка
    ...
    public Step authorStep(RepositoryItemReader<lesson42.model.sql.Author> authorReader,
        ...
        return this.stepBuilderFactory.get("authorStep")
                ...
                .writer(authorWriter)
                .listener(itemReadListener) // слушатели, определены в начале класса. Они пишут в лог то, что происходит на разных этапах
                .listener(itemWriteListener)
                .listener(itemProcessListener)
                .listener(chunkListener)
                .taskExecutor(new SimpleAsyncTaskExecutor()) // определяем порядок выполнения: синхронно или асинхронно
                .build();
    }
    ...
    public Job dbMigrationJob(
        ...
        return jobBuilderFactory.get(DB_MIGRATION_JOB)
                ...
                .start(clearDbStep) // чистка данных в монге
                .next(authorStep)// запись данных
                .next(genreStep)// запись данных
                .next(bookStep)// запись данных
                .next(commentStep)// запись данных
                .listener(jobExecutionListener) // логирование процесса работы
                .build();
    }
}